package	 auction.optimisation.period.randomPeriod;
...
public class Main extends SimulationProcess{
	...
	public void run(){
		...
		scenGen.createBidders(0, 0, 0, 50);													//Create bidders
		...
		OptimisedBidders.get(0).setPeriod(1);
		OptimisedBidders.get(1).setPeriod(5);
		OptimisedBidders.get(2).setPeriod(10);
		OptimisedBidders.get(3).setPeriod(15);
		OptimisedBidders.get(4).setPeriod(20);
		...
		ArrayList<OptimisedBidder> tempBidders = new ArrayList<OptimisedBidder>();
		for(int i=0; i<OptimisedBidders.size(); i++){
			OptimisedBidder optimisedBidder = OptimisedBidders.get(i);
		...
		for(int i=0; i<tempBidders.size(); i++){
			OptimisedBidder OptimisedBidder = tempBidders.get(i);
